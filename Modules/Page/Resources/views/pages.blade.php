            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr role="row" id="row_{{ $page->id }}" class="odd">
                        ...
                        <td>
                            @if(Sentinel::getUser()->hasAccess(['pages_write']))
                            <a class="btn btn-light active btn-xs" href="{{ route('edit-pages',['id'=>$page->id]) }}"><i class="fa fa-edit"></i>
                                {{ __('edit') }}
                            </a>
                            @endif
                            @if(Sentinel::getUser()->hasAccess(['pages_delete']))
                            <a href="javascript:void(0)" class="btn btn-light active btn-xs" onclick="delete_item('pages','{{ $page->id }}')"><i class="fa fa-trash"></i>
                                {{ __('delete') }}
                            </a>
                            @endif
                        </td>
